 ...
 */
public class KommunPolygonService {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      ...
      new KommunPolygonService().animate(df.parse("2014-09-28 23:59").getTime(), System.currentTimeMillis() + 1000 * 60 * 60 * 24, 1000 * 60 * 60 * 24);
  ...
  public KommunPolygonService() throws Exception {
  ...
  public void animate(long timestampFrom, long timestampTo, long interval) throws Exception {
    ...
    String template = IOUtils.toString(new FileInputStream("src/main/webapp/map/kommuner/animation-template.html"), "UTF8");
    ...
    for (long timestamp = timestampFrom; timestamp <= timestampTo; timestamp += interval) {
      ...
      Writer writer = new OutputStreamWriter(new FileOutputStream("src/main/webapp/map/kommuner/" + df.format(new Date(timestamp)) + ".html"), "UTF8");
      writer.write(html);
      writer.close();
    }
  ...
  public Map<String, List<Polygon>> factory() throws Exception {
    ...
    for (LocationSample locationSample : Insamlingsappen.getInstance().getPrevayler().prevalentSystem().getLocationSamples().values()) {
      if (locationSample.getCoordinate() != null
          ...
          && locationSample.getTag("ref:se:kommun") != null
        ...
        String postalTown = locationSample.getTag("ref:se:kommun").trim().toUpperCase();
